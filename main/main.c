...
static void example_handle_gen_onoff_msg(esp_ble_mesh_model_t *model, esp_ble_mesh_msg_ctx_t *ctx,
...
void example_ble_mesh_send_gen_onoff_set(void)
{
    ...
    if (err)
    {
        ESP_LOGE(TAG, "Send Generic OnOff Set Unack failed (err %d, net_idx 0x%04x, app_idx 0x%04x, dst 0x%04x)", err,
                 common.ctx.net_idx, common.ctx.app_idx, common.ctx.addr);
        return;
    }
...
{
    ESP_LOGI(TAG, "Generic client, event %u, error code %d, opcode is 0x%04" PRIx32, event, param->error_code,
             param->params->opcode);
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    ESP_LOGI(TAG, "event 0x%02x, opcode 0x%04" PRIx32 ", src 0x%04x, dst 0x%04x", event, param->ctx.recv_op,
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT)
    {
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
            ...
            ESP_LOGI(TAG, "elem_addr 0x%04x, app_idx 0x%04x, cid 0x%04x, mod_id 0x%04x",
                     ...
                     param->value.state_change.mod_app_bind.app_idx, param->value.state_change.mod_app_bind.company_id,
...
void app_main(void)
{
    ...
    while (1)
    {
        vTaskDelay(pdMS_TO_TICKS(5000)); // Čekaj 5 sekundi

        // Provjeri je li uređaj provajzan (da ima dodijeljen AppKey)
        if (store.net_idx != ESP_BLE_MESH_KEY_UNUSED && store.app_idx != ESP_BLE_MESH_KEY_UNUSED)
        {
            ESP_LOGI(TAG, "Testni ping: Šaljem OnOff naredbu na 0xC001...");
            example_ble_mesh_send_gen_onoff_set();
        }
    }
}
